...
class UploadService
{
    ...
    public function __construct(private readonly Filesystem $fileSystem,  readonly string $uploadDir)
    {
    }
    ...
    public function deleteBookFile(int $id, $fileName): void
    {
        $uploadedPath = $this->getUploadPathForBook($id) . DIRECTORY_SEPARATOR . $fileName;

        $this->fileSystem->remove($uploadedPath);
    }
